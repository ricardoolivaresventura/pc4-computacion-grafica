...
pygame.font.init()
pygame.mixer.init() # Doesn't work in google colab, because colab cannot access to audio hardware
...
# Función para aplicar la segmentación de silueta
def apply_silhouette_segmentation(frame):
    ...
    # Encuentra el contorno más grande (la silueta de la mano)
    if contours:
        hand_contour = max(contours, key=cv2.contourArea)
        
        # Dibuja la silueta de la mano sobre la imagen original
...
def main(win):
    ...
    while run:
        ...
        # Aplica la segmentación de silueta a la imagen
        img = apply_silhouette_segmentation(img)

        if results.multi_hand_landmarks:
            hand_landmarks = results.multi_hand_landmarks[0]  # Tomar solo la primera mano detectada
            # fingers_up = [lm.y < hand_landmarks.landmark[mpHands.HandLandmark.THUMB].y for lm in hand_landmarks.landmark[1:5]]
            fingers_up = [lm.y < hand_landmarks.landmark[0].y for lm in hand_landmarks.landmark[1:5]]

            if fingers_up == [1, 0, 1, 1]:  # Índice arriba, los demás abajo (izquierda)
                current_piece.x -= 1
                if not valid_space(current_piece, grid):
                    current_piece.x += 1

            elif fingers_up == [1, 1, 1, 1]:  # Todos los dedos arriba (derecha)
                current_piece.x += 1
                if not valid_space(current_piece, grid):
                    current_piece.x -= 1

            elif fingers_up == [1, 1, 0, 1]:  # Pulgar, índice y meñique arriba, los demás abajo (rotación)
